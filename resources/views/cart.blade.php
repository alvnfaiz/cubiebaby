      ...
      <div class="w-3/4 bg-white px-10 py-10">
        ...
        <div class="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5" id="cart-{{ $barang->product->id }}">
          ...
          <div class="flex justify-center w-1/5">
            ...
            <input id="{{ $barang->product->id }}" onchange="update({{ $barang->product->id }})" class="mx-2 border text-center w-8" type="text" value="{{ $barang->total_product }}">
          ...
          <span class="text-center w-1/5 font-semibold text-sm" id="total-price-{{ $barang->product->id  }}">Rp. {{ number_format($barang->product->price * $barang->total_product,2,',','.') }}</span>
        </div>
        @endforeach
